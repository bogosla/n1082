...
//     cJSON *montant = NULL;
//     cJSON *vendor_serie = NULL;
//     int x = 0, wscreen = 0, hscreen = 0, y = 0, status = 0;

//     IDirectFBSurface *prnSurface = NULL;
...
//         prnSurface = prnGetSurface();
//         prnSurface->GetSize(prnSurface, &wscreen, &hscreen);
//         x = wscreen / 2;
//         y = 20;

// 		read_from_file(USERNAME_FILE, &userData);

//     	user = cJSON_Parse(userData);


// 		company_name = cJSON_GetObjectItemCaseSensitive(json, "bank_name");
//         vendor_serie = cJSON_GetObjectItemCaseSensitive(json, "vendor_serie");
// 		ficheDate = cJSON_GetObjectItemCaseSensitive(json, "ticket_date");
// 		montant = cJSON_GetObjectItemCaseSensitive(json, "montants");

//         pntDrawPicture(x - (176 / 2), y, 176, 144, "/home/user0/mgnco/logo_48.png");
//         prnLeftIndent(x);
//         prnPrintf("%s\n", company_name->valuestring);
//         prnPrintf("%s\n", vendor_serie->valuestring);
//         prnPrintf("%s\n", ficheDate->valuestring);
//         prnStep(5);
//         prnStart();
...
int print_fiche(const char *buffer, int hasMultiple)
{
	...
	} else 
	{
		...
		if (0 == retval)
		{
			...
			ficheDate = cJSON_GetObjectItemCaseSensitive(json, "ticket_date");
			agentId = cJSON_GetObjectItemCaseSensitive(json, "vendor_name");
			company_name = cJSON_GetObjectItemCaseSensitive(json, "bank_name");
		...
		// 	{
		// 		setPrnY(y);
		// 		printerprintf(ALG_CENTER, surface, "Agent: %s", username->valuestring);
		// 		y = getPrnY();
		// 	}
